...
If any changed files are under `archiver_stats/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `archiver_stats/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `archiver_stats/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `archiver_stats/commands/main.py`,
...
For Python files, strip the `archiver_stats/` prefix and replace `/` with `.` (like module imports).

- Python file `archiver_stats/media.py` → `media:`.
- Multiple files under `archiver_stats/commands/` → `commands:`.
- Single command file `archiver_stats/commands/admin.py` → `commands.admin:`.
