...
Fixes mypy type errors and eliminates `Any` usage in the archiver-stats project.
...
1. Fix all mypy errors reported by `uv run mypy archiver_stats`
...
Place new TypedDict classes in `archiver_stats/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`archiver_stats/typing.py` when shared.
...
1. Run `uv run mypy archiver_stats` and capture all errors.
...
4. Run `uv run mypy archiver_stats` again to verify fixes.
